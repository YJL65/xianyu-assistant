...
本项目是在开源项目 [cv-cat/XianYuApis](https://github.com/cv-cat/XianYuApis) 的闲鱼第三方 API 和 WebSocket 私信通信能力基础上进行的二次开发。`vendor/XianYuApis` 负责底层闲鱼登录、签名、消息收发、商品信息获取等能力；本仓库主要新增了面向服务咨询场景的业务层，包括 AI 客服回复、飞书通知、SQLite 会话存储、人工接管、mock 调试、数据清理和测试用例。

出于依赖体积、授权边界和本地配置安全考虑，`vendor/XianYuApis` 默认不随本仓库上传。真实闲鱼模式运行前，需要按 README 的环境准备说明在本地放置该基础项目代码，并配置 `XIANYU_VENDOR_PATH`。
